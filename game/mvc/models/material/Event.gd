...
static func event_name(id_event: int) -> String:
...
# Récupère la description de l'événement en fonction de l'id_event
static func event_description(id_event: int) -> String:
	var query = "SELECT name FROM Events WHERE id_event = ?"
	var result = Utils.db.get_entries(query, [id_event])
	if result.size() > 0:
...
static func event_choice(id_event: int) -> Array:
...
static func choice_consequences(id_choice: int) -> Array:
